...
<div class="container pick-container w-800 py-30 px-50 my-50">
	<div class="cell center">
		<h1>${qnaMemberNickVO.qnaNo}번 글 보기</h1>
	</div>
	
	<div class="cell">
		<h2>
			${qnaMemberNickVO.qnaTitle}
			
			<%-- (추가) 수정시각 유무에 따라 수정됨 표시 --%>
			<c:if test="${qnaMemberNickVO.qnaEdit != null}">
				(수정됨)
			</c:if>
		</h2>
	</div>
	...
	<div class="cell">
		<h3>
			...
			<c:choose>
				<c:when test="${qnaMemberNickVO.memberId == null}">
					${qnaMemberNickVO.memberIdStr}
				</c:when>
				<c:otherwise>
...
<hr style="border: none; height: 3px; background: linear-gradient(to right, #000000, #ffffff);">
	...
	<div class="cell" style="min-height:250px;">
		...
		--%>
		${qnaMemberNickVO.qnaContent}
	</div>
	
<hr style="border: none; height: 3px; background: linear-gradient(to left, #000000, #ffffff);">

	<div class="cell">
		조회수 ${qnaMemberNickVO.qnaHits} 
	</div>
	
	<div class="cell">
		<fmt:formatDate value="${qnaMemberNickVO.qnaWrite}" 
									pattern="yyyy-MM-dd HH:mm:ss"/>
	</div>
	
	<div class="cell">
		${qnaMemberNickVO.qnaWriteDiff}
	</div>
	
	<div class="cell right">
		<a class="btn" href="write">글쓰기</a>
		<c:if test="${sessionScope.loginLevel == '관리자'}">
				<a class="btn" href="write?qnaTarget=${qnaMemberNickVO.qnaNo}">답글쓰기</a>
		</c:if>
		<%-- 
		...
		--%>
		<c:if test="${sessionScope.loginId != null && (sessionScope.loginId == qnaMemberNickVO.memberId || sessionScope.loginLevel == '관리자')}">
		<a class="btn negative" href="edit?qnaNo=${qnaMemberNickVO.qnaNo}">글수정</a>
		<a class="btn negative link-confirm btn-click" 
				data-message="정말 삭제하시겠습니까?" 
				href="delete?qnaNo=${qnaMemberNickVO.qnaNo}">글삭제</a>
		</c:if>
